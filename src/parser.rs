...
impl Parser {
    ...
    /// This is logic that is shared between from_files and from_git_repo. It allows
    /// callers to provide a custom file parsing callback and control passed in state.
    ///
    /// Note that this function *does not* validate blocks across files. This is done
    /// to support the Git repo case where validation can fail if a file/block has not
    /// been staged.
    ///
...
impl Parser {
    ...
    pub fn validate_git_repo(&self) -> Result<Vec<OnChangeViolation<'_>>> {
        ...
        for path in &staged_files {
            // Note that _all_ blocks in the file have changed, because we already filtered
            // blocks out during file parsing using staged hunks.
